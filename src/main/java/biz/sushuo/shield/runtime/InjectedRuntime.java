...
public final class InjectedRuntime {
    ...
    private static int vmProgramId(Object[] program) {
        ...
        int index = marker.intValue() == 0x53535234 ? 8 : marker.intValue() == ENCODED_MARKER ? 6 : -1;
    ...
    private static Object[] _rcmp(Class<?> caller,
        ...
        int local = nativeMix(key ^ encrypted.length ^ 0x4D43444D);
        for (int i = 0; i < encrypted.length; i++) {
            local = methodMetaStream(local, i);
            encrypted[i] = (byte) (encrypted[i] ^ (local >>> 24));
        }
        int expected = checksum(encrypted) ^ nativeMix(key ^ Integer.rotateLeft(encrypted.length * 0x45D9F3B, 7) ^ 0x4348454B);
        int invokeOpcode = flags ^ nativeMix(key ^ 0x4F50434F);
    ...
    private static int methodMetaKey(int seed, int salt, String owner, String name, String descriptor) {
        int value = nativeMix(seed ^ owner.hashCode() ^ 0x4D434B31);
        value ^= Integer.rotateLeft(name.hashCode(), 7);
        value ^= Integer.rotateLeft(descriptor.hashCode(), 13);
        value = nativeMix(value ^ salt ^ owner.length() * 0x45D9F3B);
        value ^= Integer.rotateLeft(descriptor.length() * 0x27D4EB2D, 9);
        return nativeMix(value ^ 0x4D434B32);
    }
    ...
    private static int checksum(byte[] bytes) {
        ...
        return nativeMix(value ^ bytes.length);
    }

    private static Class<?> ownerRuntimeClass() {
        return InjectedRuntime.class;
    }
    ...
    private static Object _j(Object[] program, Object[] args, int token) {
        int tokenFence = token ^ vmCallToken(program, args.length);
        if (tokenFence != 0) {
            state ^= tokenFence;
            throw new IllegalStateException("x");
        }
    ...
    private static String tempNativePrefix(String resource, String mapped) {
        int a = nativeMix(resource.hashCode() ^ Integer.rotateLeft(mapped.hashCode(), 7) ^ 0x544D5031);
        int b = nativeMix(a ^ resource.length() * 0x45D9F3B ^ mapped.length() * 0x27D4EB2D);
    ...
    private static String tempNativeSuffix(String resource, String mapped) {
        int a = nativeMix(resource.hashCode() ^ Integer.rotateLeft(mapped.hashCode(), 11) ^ 0x544D5032);
    ...
    private static int nativeFormat(int resourceHash, int runtimeHash, int totalLength) {
        ...
        return nativeMix(value ^ 0x7F4A7C15);
    }
